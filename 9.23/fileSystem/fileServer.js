...
function showList(res) {
    ...
    for (var i = 0; i < files.length; i++) {
        ...
        fileObj.createTime = brithTimer.getFullYear() + "-" + (brithTimer.getMonth() + 1) + "-" + brithTimer.getDate() + "-" + brithTimer.getHours() + ":" + brithTimer.getMinutes() + ":" + brithTimer.getSeconds();
        list.push(fileObj);
    }
...
function getFile(urlObj,res) {
    ...
    for(var i = 0; i< list.length; i++){
        if(list[i].createtime == createTime) {
            showContent(list[1].fileName, res);
        }
    }
}
